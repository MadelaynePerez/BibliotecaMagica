...
public class Grafo {
    ...
    public static enum Criterio { TIEMPO, COSTO }
    ...
    public static class Arista {
        ...
        public Arista(String destino, int tiempo, int costo) {
            this.destino = destino; this.tiempo = tiempo; this.costo = costo;
        }
    }
    ...
    public ResultadoRuta dijkstra(String origen, String destino, Criterio criterio) {
        if (!ady.containsKey(origen) || !ady.containsKey(destino)) return ResultadoRuta.vacio();

        Map<String,Integer> dist = new HashMap<>();
        Map<String,String>  prev = new HashMap<>();
        for (String v: ady.keySet()) dist.put(v, Integer.MAX_VALUE);
        dist.put(origen, 0);
        ...
        if (dist.get(destino)==Integer.MAX_VALUE) return ResultadoRuta.vacio();
        // reconstruir ruta
        LinkedList<String> ruta = new LinkedList<>();
        for (String at = destino; at!=null; at = prev.get(at)) ruta.addFirst(at);
    ...
    public static class ResultadoRuta {
        ...
        public ResultadoRuta(List<String> ruta, int peso, Criterio c) {
            this.ruta = ruta; this.pesoTotal = peso; this.criterio = c;
        }
        public static ResultadoRuta vacio() { return new ResultadoRuta(Collections.emptyList(), -1, Criterio.TIEMPO); }
        @Override public String toString() {
            return ruta.isEmpty()? "Sin ruta" : "Ruta: " + String.join(" -> ", ruta) + " | " + criterio + "=" + pesoTotal;
        }
    }
    ...
    public String mostrarTexto() {
        ...
        for (var e: ady.entrySet()) {
            ...
            for (var a: e.getValue()) sb.append(a.destino).append("(t=").append(a.tiempo).append(",c=").append(a.costo).append(") ");
            sb.append("\n");
        }
        return sb.toString();
    }
