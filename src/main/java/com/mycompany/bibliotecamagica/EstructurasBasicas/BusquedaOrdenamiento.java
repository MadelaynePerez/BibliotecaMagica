...
public class BusquedaOrdenamiento {
    ...
    public static Libro busquedaSecuencialPorAutor(List<Libro> lista, String autor) {
        ...
        for (Libro l : lista) {
            if (l.getAutor().toLowerCase().contains(t)) {
                return l;
            }
        }
        return null;
    ...
    public static Libro busquedaBinariaPorTitulo(List<Libro> listaOrdenadaPorTitulo, String titulo) {
        ...
        while (lo <= hi) {
            ...
            if (cmp == 0) {
                return listaOrdenadaPorTitulo.get(mid);
            }
            if (cmp < 0) {
                lo = mid + 1;
            } else {
    ...
    public static void seleccion(List<Libro> a, Comparator<Libro> cmp) {
        for (int i = 0; i < a.size(); i++) {
            int min = i;
            for (int j = i + 1; j < a.size(); j++) {
                if (cmp.compare(a.get(j), a.get(min)) < 0) {
                    min = j;
                }
            }
    ...
    public static void insercion(List<Libro> a, Comparator<Libro> cmp) {
        for (int i = 1; i < a.size(); i++) {
            Libro key = a.get(i);
            int j = i - 1;
            while (j >= 0 && cmp.compare(key, a.get(j)) < 0) {
                a.set(j + 1, a.get(j));
                j--;
            }
            a.set(j + 1, key);
    ...
    public static void shell(List<Libro> a, Comparator<Libro> cmp) {
        for (int gap = a.size() / 2; gap > 0; gap /= 2) {
            for (int i = gap; i < a.size(); i++) {
                Libro temp = a.get(i);
                int j = i;
                while (j >= gap && cmp.compare(temp, a.get(j - gap)) < 0) {
                    a.set(j, a.get(j - gap));
                    j -= gap;
                }
                a.set(j, temp);
    ...
    private static void qs(List<Libro> a, int lo, int hi, Comparator<Libro> cmp) {
        ...
        int i = lo, j = hi;
        while (i <= j) {
            while (cmp.compare(a.get(i), p) < 0) {
                i++;
            }
            while (cmp.compare(a.get(j), p) > 0) {
                j--;
            }
            if (i <= j) {
                Collections.swap(a, i, j);
    ...
    public static Comparator<Libro> porISBN() {
        return Comparator.comparing(Libro::getIsbn, String.CASE_INSENSITIVE_ORDER);
    }

    public static Comparator<Libro> porAnio() {
        return Comparator.comparingInt(Libro::getAnio);
    }
}
